...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const coreUpgrades_raw_data = [
        { id: "biggerBrain1", nameKey: "upgrades.brainGrowth1.name", descKey: "upgrades.brainGrowth1.desc", costCurrency: "neurons", cost: 10, effectApplied: false, type: 'brain', action: () => { UIManager.logMessage("biggerBrain1 ACTION TRIGGERED!", "log-info"); gameState.currentBrainLevel = 1; gameState.neuroFuelCost = calculateNextNeuroFuelCost(); UIManager.callUpdateBrainVisual();LanguageManager.apply(gameState.currentBrainLevel); if(brainVisualAreaDOM) brainVisualAreaDOM.style.display = 'block'; if(leftColumnDOM) leftColumnDOM.style.display = 'block'; if(neurofuelAreaDOM) neurofuelAreaDOM.style.display = 'block'; if(projectsAreaDOM) projectsAreaDOM.style.display = 'block'; QuestionSystem.setOverallUnlockState(true); QuestionSystem.unlockDifficultyLevel(0); if (neuronProliferationAreaDOM) { neuronProliferationAreaDOM.style.display = 'block'; UpgradeSystem.renderNeuronProliferationUpgrades(); } UIManager.logMessage("Brain Growth I: Easy Qs & Proliferation unlocked.", "log-unlock"); }},
        { id: "biggerBrain2", nameKey: "upgrades.brainGrowth2.name", descKey: "upgrades.brainGrowth2.desc", costCurrency: "neurons", cost: 250, effectApplied: false, dependsOn: "biggerBrain1", type: 'brain', action: () => { gameState.currentBrainLevel = 2; gameState.neuroFuelCost = calculateNextNeuroFuelCost(); UIManager.callUpdateBrainVisual(); LanguageManager.apply(gameState.currentBrainLevel); QuestionSystem.unlockDifficultyLevel(1); if (hypothalamusControlsAreaDOM) hypothalamusControlsAreaDOM.style.display = 'block'; UIManager.logMessage("Brain Growth II: Medium Qs & Hypothalamus unlocked.", "log-unlock"); }},
        { id: "biggerBrain3", nameKey: "upgrades.brainGrowth3.name", descKey: "upgrades.brainGrowth3.desc", costCurrency: "neurons", cost: 1000, effectApplied: false, dependsOn: "biggerBrain2", type: 'brain', action: () => { gameState.currentBrainLevel = 3; gameState.neuroFuelCost = calculateNextNeuroFuelCost(); UIManager.callUpdateBrainVisual(); LanguageManager.apply(gameState.currentBrainLevel); QuestionSystem.unlockDifficultyLevel(2); startIrregularSnacks(); UIManager.logMessage("Brain Growth III: Hard Qs & Amygdala research.", "log-unlock"); UpgradeSystem.renderCoreUpgrades(); }},
        { id: "amygdalaActivation", nameKey: "upgrades.amygdalaActivation.name", descKey: "upgrades.amygdalaActivation.desc", costCurrency: "neurons", cost: 5000, psychbuckCost: 200, effectApplied: false, dependsOn: "biggerBrain3", type: 'brain', action: () => { gameState.passiveNeuronsPerSecond = (gameState.passiveNeuronsPerSecond > 0 ? gameState.passiveNeuronsPerSecond : 0.1) * 2; AnxietySystem.activateAmygdala(); UIManager.logMessage("Amygdala activated! Production boosted.", "log-unlock"); /* UIManager.updateAllDisplays(); // Called by purchaseUpgrade */ }},
        { id: "nucleusAccumbens", nameKey: "upgrades.nucleusAccumbens.name", descKey: "upgrades.nucleusAccumbens.desc", costCurrency: "neurons", cost: 2000, effectApplied: false, dependsOn: "biggerBrain3", type: 'brain', action: () => { gameState.unlockedGames.neurosnake = true; gameState.unlockedGames.brainTetris = true; gameState.unlockedGames.flappyFreud = true; gameState.unlockedGames.feedSundgren = true; UIManager.updateMinigameButtons(); UIManager.logMessage("Pleasure center engaged! Minigames available.", 'log-unlock'); }}
    ];
    const neuronProliferationUpgrades_raw_data = [
        { id: "prolifFactory", nameKey: "upgrades.prolifFactory.name", descKey: "upgrades.prolifFactory.desc", costCurrency: "psychbucks", cost: 10, neuronBoost: 0.5, effectApplied: false, type: 'proliferation', action: () => { if(factoryAreaDOM) { factoryAreaDOM.style.display = 'block'; UIManager.updateFactoryDisplay(); } } },
        { id: "dendriticSprouting", nameKey: "upgrades.dendriticSprouting.name", descKey: "upgrades.dendriticSprouting.desc", costCurrency: "psychbucks", cost: 25, percentBoost: 0.1, effectApplied: false, dependsOn: "prolifFactory", type: 'proliferation' },
        { id: "myelination", nameKey: "upgrades.myelination.name", descKey: "upgrades.myelination.desc", costCurrency: "psychbucks", cost: 60, percentBoost: 20, extraFuel: 0.2, anxietyBoost: 5, effectApplied: false, dependsOn: "dendriticSprouting", type: 'proliferation' },
        { id: "metabolicEfficiency", nameKey: "upgrades.metabolicEfficiency.name", descKey: "upgrades.metabolicEfficiency.desc", costCurrency: "psychbucks", cost: 80, effectApplied: false, factoryRequirement: 4, type: 'proliferation', action: () => { gameState.manualFuelMultiplier *= 0.5; gameState.passiveNeuroFuelMultiplier *= 0.5; } },
        { id: "intermittentFasting", nameKey: "upgrades.intermittentFasting.name", descKey: "upgrades.intermittentFasting.desc", costCurrency: "psychbucks", cost: 120, effectApplied: false, dependsOn: "metabolicEfficiency", type: 'proliferation', action: () => { startIntermittentFasting(); } },
        { id: "metabolicEfficiency2", nameKey: "upgrades.metabolicEfficiency2.name", descKey: "upgrades.metabolicEfficiency2.desc", costCurrency: "psychbucks", cost: 200, effectApplied: false, dependsOn: "metabolicEfficiency", brainRequirement: 3, type: 'proliferation', action: () => { gameState.manualFuelMultiplier *= 0.5; gameState.passiveNeuroFuelMultiplier *= 0.5; } },
    ];
    ...
    const UIManager = {
        ...
        renderUpgradeList(upgsToRender,listElDOM,upgTypeStr){if(!listElDOM)return;listElDOM.innerHTML="";upgsToRender.forEach(upg=>{const itm=document.createElement('div');itm.classList.add('upgrade-item');const name=LanguageManager.getPhrase(upg.nameKey,gameState.currentBrainLevel);const desc=LanguageManager.getPhrase(upg.descKey,gameState.currentBrainLevel);if(upg.effectApplied){itm.classList.add('purchased');let sTxt="(Purchased)";if(upg.id==="amygdalaActivation"&&AnxietySystem.isAmygdalaFunctioning())sTxt="(Active)";itm.innerHTML=`<h3>${name} ${sTxt}</h3><p>${desc}</p>`;}else{let cTxt=`${upg.cost} ${upg.costCurrency}`;if(upg.psychbuckCost)cTxt+=` & ${upg.psychbuckCost} Psychbucks`;itm.innerHTML=`<h3>${name}</h3><p>${desc}</p><p>Cost: ${cTxt}</p><button data-upgrade-id="${upg.id}" data-upgrade-type="${upgTypeStr}">Purchase</button>`;const btn=itm.querySelector('button');if(btn)btn.onclick=(e)=>UpgradeSystem.purchaseUpgrade(e.target.dataset.upgradeId,e.target.dataset.upgradeType);}listElDOM.appendChild(itm);});},
    ...
    const UpgradeSystem = {
        ...
        purchaseUpgrade(upgradeId, upgradeType) {
            ...
            if (!upg) { UIManager.logMessage(`Upg Err: "${upgradeId}" not found.`, "log-warning"); return; } const upgName=LanguageManager.getPhrase(upg.nameKey,gameState.currentBrainLevel); if (upg.effectApplied) { UIManager.logMessage(`${upgName} already purchased.`, "log-info"); return; }
        ...
        markUpgradeNotApplied(upgradeId, upgradeType) { const source = upgradeType === "core" ? this.coreUpgrades : this.neuronProliferationUpgrades; const upg = source.find(u => u.id === upgradeId); if (upg) { upg.effectApplied = false; const upgName=LanguageManager.getPhrase(upg.nameKey,gameState.currentBrainLevel); UIManager.logMessage(`Reverted purchase status for ${upgName}.`, "log-info"); } },
